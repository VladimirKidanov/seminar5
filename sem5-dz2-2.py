# ИГРА В КОНФЕТЫ против Бота

from random import randint, choice

greeting = ('Правила игры: '
    'Нам будет дано некоторое количество конфет, '
    'за один ход можете взять не более определённого количества, '
...
def get_rules(players):
    ...
    first = int(input(
        f'{players[0]}, если хотите ходить первым, нажмите 1, если нет, любую другую цифру\n'))
    if first != 1:
        first = 0
    ...
    while rules[0] > 0:
        if not count % 2:
            ...
            if move > rules[0] or move > rules[1]:
                print(
                    f'Не жадничай, можно взять не более {rules[1]} конфет{letter}, у нас всего {rules[0]} конфет{letter}')
                attempt = 3
                while attempt > 0:
...
if not winer:
    ...
    print(
        f'Победил {winer}! Ему достаются все конфеты!')